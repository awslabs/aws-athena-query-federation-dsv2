...
public interface AthenaFederationAdapterDefinition
{
    ...
    {
        // Forcefully disable spilling by overwriting any thing the user attempts to
        // set for `spill_bucket` and `spill_prefix` and then `disable_spill_encryption`
        // because otherwise we will run into issues with some Spark distributions.
        Map<String, String> spillConfigOverrides = com.google.common.collect.ImmutableMap.of(
        ...
        // Also filter out all the spillConfigOverrides. We are purposefully doing this rather than
        // relying on the Collectors.toMap overwriting the values in the right order.
        Stream<? extends Map.Entry<String, String>> userFederationConfigStream = configOptions.entrySet().stream()
            ...
            .filter(e -> !spillConfigOverrides.containsKey(e.getKey()));

        Stream<Map.Entry<String, String>> federationConfigOverridesStream = spillConfigOverrides.entrySet().stream();

        return Stream.concat(userFederationConfigStream, federationConfigOverridesStream)
